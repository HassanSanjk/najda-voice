...
| Dynamic DNS | **DuckDNS** | Free; solves EC2 issuing a new IP every boot. |
| Safety net | **CloudWatch auto-stop + AWS Budgets alert** | Planned (Day 11 of the original build plan) but **not yet confirmed implemented** — this fell behind once Telnyx/Arabic debugging consumed most of the later sessions. Flag as an open item. |
...
1. **EC2 deployment** not yet done. Planned: `us-east-1`, t2/t3.micro, Docker path fully prepared (including `docker-compose.yml` forcing `APP_ENV=production`); DuckDNS repoint keeps the webhook URL unchanged; add ~1GB swap before the first Docker build on a micro instance. Realistic latency gain once deployed: an estimated **0.4–0.8 seconds per turn** (removes the Malaysia↔US network round trip that every single API call currently pays during local dev testing — Deepgram proximity specifically; Groq already appears to serve from `bom`/`syd` regions per its own response headers). The latency floor that will remain regardless of deployment location: ~1 second `utterance_end_ms` endpointing + 0.5–2 seconds of first-sentence TTS synthesis time (this is Deepgram/Groq's own processing cost, not something code can remove).
2. **CloudWatch auto-stop + AWS Budgets billing alert** (Day 11 of the original plan) — status unconfirmed; likely still pending, since later sessions were consumed by Telnyx/Arabic debugging instead of returning to this.
...
6. **§5 (naturalness/repetition fix) is implemented and live-tested** — the Aug 2 call confirmed the naturalness behavior but surfaced two new bugs (Arabic scenario misroute, verbatim triage repeat) now fixed in §4.29. After the **follow-up live re-test** of those fixes passes and §6 is updated, the natural next item is §7.1 (EC2 deployment).
...
**Production (EC2):** `bash scripts/start.sh` — wraps `run.py` with logging/readiness checks. `docker-compose.yml` forces `APP_ENV=production`. See §7.1 for the deployment plan (add ~1GB swap before the first Docker build on a micro instance).
