...
| TTS (English) | **Groq Orpheus English** (`canopylabs/orpheus-v1-english`) — *originally Deepgram Aura-2, changed Aug 2026* | Consolidated English onto the same Orpheus provider as Arabic (one SDK, one client, one semaphore, one WAV→mu-law path) — a decision, not just a swap. Rationale: Deepgram English showed a ~1.3-1.6s fixed TTS floor vs ~0.4-0.9s for Groq Arabic; the §4.30 degradation investigation found **both** providers independently showing the same staircase-latency symptom, pointing at how `voice.py` calls TTS generically rather than at provider internals (one code path deliberately shrinks that surface); and two providers meant two leak/throttle surfaces plus an audio-format asymmetry. `deepgram_tts.py` stays as a **runtime** rollback — `TTS_PROVIDER_EN=deepgram` (default `groq`), symmetric with `TTS_PROVIDER_AR` (see §10). Voice via `GROQ_TTS_VOICE_EN` (default `austin`). Deepgram remains for STT only. |
...
│   └── models/schemas.py
│
├── knowledge/                      # 13 scenarios (trauma/stroke/seizures/poisoning/drowning added)
...
22. **KB scenario-matching bugs** (`app/prompts/kb_loader.py`):
    ...
    - **Remaining known limitations:** matching is still keyword-based (an LLM-side scenario classifier would be the proper structural upgrade if more misroutes surface); the matched scenario is "sticky" for the rest of a call once set; **KB coverage was expanded from 8 to 13 scenarios — trauma/road accidents, stroke, seizures, poisoning, and drowning now all have KB files** — anything still uncovered falls through to the generic router (which safely defers to real emergency services, but offers no specific guidance). Authoring more `knowledge/KB_*.yaml` files is a pure content task; the schema itself is proven and stable.
...
7. **KB breadth — resolved.** Coverage expanded from 8 to 13 scenarios; trauma/road accidents, stroke, seizures, poisoning, and drowning all have KB files now. Further scenarios remain a pure content task (schema is proven).
...
| `TTS_PROVIDER_EN` | `"groq"` | `"groq"` (Orpheus English, default) or `"deepgram"` (Aura-2 runtime rollback) — mirrors the AR knob so both languages have a real fallback path | `language.py::get_tts_provider()`, `deepgram_tts.py` |
